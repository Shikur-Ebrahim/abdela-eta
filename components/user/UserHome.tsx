...
import { LotteryRound, PaymentMethod, WinnerSettings, TelegramSettings } from "../../types";
import WinnerReveal from "./WinnerReveal";

interface Props {
    lotteries: LotteryRound[];
    paymentMethods: PaymentMethod[];
    winnerSettings: WinnerSettings | null;
    telegramSettings: TelegramSettings | null;
}

export default function UserHome({ lotteries, paymentMethods, winnerSettings, telegramSettings }: Props) {
    const { t } = useLanguage();
    ...
    return (
        <main className="mx-auto max-w-7xl px-1 sm:px-4 pt-0 pb-8 md:px-8 lg:px-12">
            ...
            {winnerSettings?.isActive ? (
                <div className="min-h-[80vh] flex flex-col justify-center py-10">
                    <WinnerReveal 
                        winnerNumber={winnerSettings.winnerNumber} 
                        duration={winnerSettings.revealDuration} 
                        imageId={winnerSettings.imageId}
                        carTitle={winnerSettings.carTitle}
                        telegramUsername={winnerSettings.telegramUsername || telegramSettings?.supportUsername}
                    />
                </div>
            ) : (
                <>
